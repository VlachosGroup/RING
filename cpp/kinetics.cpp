...
using std::vector; using std::multimap; using std::pair; using std::map; 
...
KineticsInfo::KineticsInfo(vector<KineticParamPtr>& kinetics, generated_rxn& reaction, 
        double GasConst, double T, double H, double S, double n, double d, int r, int p)
{
...
KineticsInfo::KineticsInfo(vector<AltKineticParamPtr>& kinetics, vector<double>* params, 
        generated_rxn& reaction, double GasConst, double T, double H, double S, double n)
{
...
bool KineticsInfo::getKineticParameters( double& PreExp, double& ActE, double& TempIndex, 
        double& kinValue, bool& calcK, bool& usesBEP, bool& usesLFER, double& alpha, 
        double& beta, bool& stick)
{
	//TODO:: The tasks involved in calculating parameters defined through reverse reaction 
    //have unnecessarily repetitive steps. This is done in purpose to be sure they are correct! 
	...
	if (!KineticFunctions->at(rule)(reactants,products,PreExp,TempIndex, ActE, 
                calcK, isBEP, usesLFER, alpha,beta,stick,Temp))
	{
	...
	//if (calcK || isBEP) RxnEnthalpy = Network->calculateThermoOfRxn(EnthalpyType,
    //Network->AllReactions[i],Network->Temperature)*1000.0;
...
bool KineticsInfo::getKineticParameters(double& kinValue, int& kIndex, double& kRelative, 
        int& EaIndex, double& EaRelative,double& RefTemp, double& TempExp, bool& calcK)
{
